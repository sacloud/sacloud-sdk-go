...
type dummyDiskBuilder struct {
	...
	diskID       types.ID
	updateLevel  service.UpdateLevel
	noWait       bool
	err          error
}
...
func (d *dummyDiskBuilder) UpdateLevel(ctx context.Context, zone string, disk *iaas.Disk) service.UpdateLevel {
...
func TestBuilder_Build_BlackBox(t *testing.T) {
	...
	testutil.RunCRUD(t, &testutil.CRUDTestCase{
		...
		Shutdown: func(ctx *testutil.CRUDTestContext, caller iaas.APICaller) error {
			return power.ShutdownServer(ctx, iaas.NewServerOp(caller), testZone, ctx.ID, true)
		},
		Delete: &testutil.CRUDTestDeleteFunc{
			Func: func(ctx *testutil.CRUDTestContext, caller iaas.APICaller) error {
				serverOp := iaas.NewServerOp(caller)
				if err := serverOp.DeleteWithDisks(ctx, testZone, ctx.ID, &iaas.ServerDeleteWithDisksRequest{IDs: diskIDs}); err != nil {
					return err
				}
...
func TestBuilder_UpdateWithPreviousID(t *testing.T) {
	...
	serverOp := iaas.NewServerOp(testutil.SingletonAPICaller())
