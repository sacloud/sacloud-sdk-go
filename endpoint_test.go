...
func (this *endpointTest) TestEndpointConfig_FromCommandLine() {
	this.setEnv("SAKURA_ACCESS_TOKEN", "test-token")
	this.setEnv("SAKURA_ACCESS_TOKEN_SECRET", "test-secret")
	...
	err = api.SetEnviron(nil)
	this.NoError(err)

	fs := api.FlagSet(0)
	err = fs.Parse([]string{
		"--endpoints", `{"iaas":"https://cli.example.com/cloud/zone","iam":"https://cli.example.com/cloud/api/iam/1.0"}`,
	})
	this.NoError(err)
	...
	this.NotNil(cfg)
	this.NotNil(cfg.Endpoints)
	this.Equal("https://cli.example.com/cloud/zone", cfg.Endpoints["iaas"])
	this.Equal("https://cli.example.com/cloud/api/iam/1.0", cfg.Endpoints["iam"])
}

func (this *endpointTest) TestEndpointConfig_CLIOverridesEnv() {
	this.setEnv("SAKURA_ACCESS_TOKEN", "test-token")
	this.setEnv("SAKURA_ACCESS_TOKEN_SECRET", "test-secret")
	this.setEnv("SAKURA_ENDPOINTS_IAAS", "https://env.example.com/cloud/zone")
	this.setEnv("SAKURA_ENDPOINTS_IAM", "https://env.example.com/cloud/api/iam/1.0")
	...
	fs := api.FlagSet(0)
	err = fs.Parse([]string{
		"--endpoints", `{"iaas":"https://cli.example.com/cloud/zone"}`,
	})
	this.NoError(err)
	...
	this.Equal("https://cli.example.com/cloud/zone", cfg.Endpoints["iaas"])
	// IAM from environment should remain (not overwritten by CLI)
	this.Equal("https://env.example.com/cloud/api/iam/1.0", cfg.Endpoints["iam"])
}

func (this *endpointTest) TestEndpointConfig_CLIInvalidJSON() {
	this.setEnv("SAKURA_ACCESS_TOKEN", "test-token")
	this.setEnv("SAKURA_ACCESS_TOKEN_SECRET", "test-secret")
	...
	fs := api.FlagSet(0)
	err = fs.Parse([]string{
		"--endpoints", `{not valid json`,
	})
	this.Error(err)
	this.Contains(err.Error(), "invalid JSON for endpoints")
}
