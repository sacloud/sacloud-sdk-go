...
var jsonFieldsNameOfCommonServiceItemIcon = [4]string{
	0: "ID",
	1: "URL",
	2: "Name",
	3: "Scope",
	...
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfCommonServiceItemIcon) {
					name = jsonFieldsNameOfCommonServiceItemIcon[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
...
func (o NilCommonServiceItemIcon) Encode(e *jx.Encoder) {
	if o.Null {
		e.Null()
		return
	}
...
// Decode decodes CommonServiceItemIcon from json.
func (o *NilCommonServiceItemIcon) Decode(d *jx.Decoder) error {
	if o == nil {
		return errors.New("invalid: unable to decode NilCommonServiceItemIcon to nil")
	}
	if d.Next() == jx.Null {
...
// MarshalJSON implements stdjson.Marshaler.
func (s NilCommonServiceItemIcon) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	s.Encode(&e)
	return e.Bytes(), nil
}
...
// UnmarshalJSON implements stdjson.Unmarshaler.
func (s *NilCommonServiceItemIcon) UnmarshalJSON(data []byte) error {
	d := jx.DecodeBytes(data)
	return s.Decode(d)
}
